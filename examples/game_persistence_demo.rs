...
impl<'a> Circle<'a> {
    fn new(
        id: usize, x: i16, y: i16, r: i16, xvel: i16, yvel: i16, color: Color,
        tc: &'a TextureCreator<WindowContext>,
    ) -> Result<Circle<'a>, String> {
        let rr = (2 * r) as u32 + 1;
    ...
    fn new_random(
        ctx: &Context, id: usize, tc: &'a TextureCreator<WindowContext>,
    ) -> Result<Circle<'a>, String> {
        let (r, g, b) =
            (rand::random::<u8>() % 255, rand::random::<u8>() % 255, rand::random::<u8>() % 255);
        ...
        Circle::new(
            id,
            (rand::random::<u16>() % 512 + 5) as i16,
            (rand::random::<u16>() % 512 + 5) as i16,
            (rand::random::<u16>() % 10 + 5) as i16,
            rand::random::<i16>() % 10 + 2,
            rand::random::<i16>() % 10 + 2,
            Color::RGB(r, g, b),
            tc,
        )
    ...
    fn tick<T: sdl2::render::RenderTarget>(
        &mut self, delta: i16, canvas: &mut Canvas<T>,
    ) -> Result<(), String> {
...
fn main() {
    ...
    let window =
        video.window("YottaDB Persistence Demo", 1024, 768).position_centered().build().unwrap();
    ...
    for ball in balls_key.iter_key_subs() {
        ...
        circles.push(
            Circle::new(id, x, y, radius, xvel, yvel, Color::RGB(r, g, b), &texture_creator)
                .unwrap(),
        );
    }
    ...
    'running: loop {
        ...
        for event in event_pump.poll_iter() {
            match event {
                ...
                Event::KeyDown { keycode: Some(Keycode::N), .. } => {
                    circles.push(
                        Circle::new_random(&context, circles.len(), &texture_creator).unwrap(),
                    );
                }
                _ => {}
            }
        ...
        let surface = font
            .render(&format!("SPF: {}", elapsed_time))
        ...
        let surface = font
            .render(&format!("Num Objects: {}", circles.len()))
            .blended(Color::RGBA(255, 0, 0, 255))
            .unwrap();
        let texture = texture_creator.create_texture_from_surface(&surface).unwrap();
